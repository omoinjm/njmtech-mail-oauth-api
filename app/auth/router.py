...
async def login_to_google():
    login_url = GoogleOAuth2.get_login_url()
...
async def google_callback(code: str, db: AsyncSession = Depends(get_db)) -> AuthResponse:
    try:
        ...
        token_payload = await GoogleOAuth2.exchange_code_for_tokens(code)
        ...
        if not access_token or not expires_in:
            raise HTTPException(status_code=400, detail="Invalid token data from Google")

        # Get user info from Google
        user_info_payload = await GoogleOAuth2.get_user_info(access_token)
        ...
        await repo.create_or_update_user_with_token(user_info, token_data, "google")
        ...
        raise HTTPException(status_code=500, detail="An unexpected error occurred during Google OAuth callback.")
